...
from experiments.simulation import generate_simulator_with_parameters, run_simulation_to_completion
from prefopt.edward_optimizer import QueueRunner, add_to_queue, timeout_handler, TimeoutError, TensorFlowError


@add_to_queue
def _run_simulation_in_queue_for_real(parameters, max_steps):
...
def _run_simulation_with_queue(parameters, max_steps, timeout_factor=0):
    ...
    q = Queue()
    p = QueueRunner(target=_run_simulation_in_queue_for_real, args=(q, parameters, max_steps))
    p.start()
    p.join()
    if p.exception:
        raise p.exception
...
def run_simulation_with_queue(parameters, max_steps, timeout=True):
    x_next = None
    ...
    while x_next is None:
        try:
            x_next = _run_simulation_with_queue(parameters, max_steps, timeout_factor)
        except TimeoutError:
